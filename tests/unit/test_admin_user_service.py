...
- систему нельзя оставить без активного администратора;
...
async def test_authenticate_rejects_user_without_password():
    """У пользователя только внешний вход — пароля нет."""
...
async def test_external_login_requires_linked_account():
    """Аккаунт провайдера должен быть привязан заранее, иначе доступа нет."""
    service, _, _, _ = _service()
...
async def test_external_login_works_for_linked_account():
    service, _, _, _ = _service(
        identities=[
            AdminIdentity(
                id=1, user_id=1, provider=AuthProvider.YANDEX, provider_user_id="ya-1"
            )
        ]
    )
...
async def test_external_login_rejects_disabled_user():
    service, _, _, _ = _service(
        ...
        identities=[
            AdminIdentity(
                id=1, user_id=1, provider=AuthProvider.VK, provider_user_id="vk-9"
            )
        ],
    )
...
async def test_password_is_not_an_external_provider():
    ...
    with pytest.raises(AdminUserError, match="не является внешним"):
        await service.link_identity(
            1, provider=AuthProvider.PASSWORD, provider_user_id="x"
        )


async def test_link_identity_records_provider_without_account_id():
    """В журнал не должен попадать идентификатор аккаунта у провайдера."""
    service, _, _, audit = _service()

    await service.link_identity(
        1, provider=AuthProvider.MAX, provider_user_id="secret-account-id"
    )
...
async def test_created_user_must_change_password_by_default():
    ...
    created = await service.create_user(
        login="newbie", password="temporary-pass-1", role=AdminRole.VIEWER
    )
...
async def test_create_user_rejects_weak_password():
    ...
    with pytest.raises(AdminUserError, match="не короче"):
        await service.create_user(
            login="newbie", password="short", role=AdminRole.VIEWER
        )
...
async def test_duplicate_login_rejected():
    ...
    with pytest.raises(ProfilePersistenceError):
        await service.create_user(
            login="owner", password="another-pass-01", role=AdminRole.VIEWER
        )
...
async def test_can_demote_admin_when_another_admin_exists():
    service, users, _, _ = _service(
        [_user(id=1, login="first"), _user(id=2, login="second")]
    )
...
async def test_cannot_delete_own_account():
    """Иначе администратор способен заблокировать сам себя."""
    service, _, _, _ = _service(
        [_user(id=1, login="first"), _user(id=2, login="second")]
    )
...
async def test_deleting_viewer_is_allowed():
    service, users, _, _ = _service(
        [_user(id=1, login="boss"), _user(id=2, login="guest", role=AdminRole.VIEWER)]
    )
...
async def test_disabled_admin_does_not_count_as_protection():
    """Отключённый админ не считается доступом: последний активный защищён."""
    service, _, _, _ = _service(
        [_user(id=1, login="active"), _user(id=2, login="off", is_active=False)]
    )
...
async def test_change_own_password_requires_current():
    ...
    with pytest.raises(AdminUserError, match="неверно"):
        await service.change_own_password(
            1, current_password="wrong-one-x", new_password="brand-new-pass-1"
        )
...
async def test_change_own_password_clears_must_change_flag():
    ...
    await service.change_own_password(
        1, current_password=PASSWORD, new_password="brand-new-pass-1"
    )
...
async def test_change_own_password_rejects_same_password():
    ...
    with pytest.raises(AdminUserError, match="отличаться"):
        await service.change_own_password(
            1, current_password=PASSWORD, new_password=PASSWORD
        )
...
async def test_change_own_password_enforces_policy():
    ...
    with pytest.raises(AdminUserError, match="не короче"):
        await service.change_own_password(
            1, current_password=PASSWORD, new_password="short"
        )
...
async def test_reset_password_is_not_logged():
    service, _, _, audit = _service()
...
async def test_audit_failure_does_not_break_operation():
    """Журнал не критичен: сбой записи не должен отменять создание."""
    ...
    service = AdminUserService(
        users=users, identities=FakeIdentities(), audit=BrokenAudit()
    )
