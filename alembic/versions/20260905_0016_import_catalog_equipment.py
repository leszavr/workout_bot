...
Область применения ограничена: миграция обрабатывает данные, которые уже лежат в
таблице `exercises` на момент её применения. В чистом окружении порядок
обратный — сначала `alembic upgrade head`, потом импорт каталога, — и тогда
миграция работает по пустой таблице и не делает ничего. Это не дефект: то же
сопоставление выполняет `scripts/build_equipment_knowledge.py`, который
идемпотентен и запускается после импорта каталога (и в CI, и при развёртывании).
Миграция закрывает переход для окружений, где каталог уже есть.
...
миграцию не входят — они живут в `src/application/equipment/` и запускаются тем
же скриптом, потому что их результат пересчитывается при пополнении словаря.
