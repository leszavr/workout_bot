...
Redis остался нужен и после выноса Gateway за сетевую границу: FSM-middleware
aiogram читает состояние на каждом обновлении, а изоляция сериализует
параллельные обновления одного пользователя. При недоступном Redis обновление
падает до хендлера, и без этого обработчика сбой выглядит для пользователя как
«бот не отвечает» — aiogram логирует исключение и продолжает polling, а
сообщение исчезает.

Анкета при этом не теряется: позиция и ответы лежат в PostgreSQL (RU), поэтому
после восстановления Redis диалог продолжается с того же места.
"""
