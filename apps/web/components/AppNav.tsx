...
const NAV = [
  ...
  // «Упражнения» — что можно выполнять, «База знаний» — чем и на чём. Разделы
  // соседние, но отвечают на разные вопросы, и словарь оборудования не является
  // частью каталога.
  { href: "/knowledge", label: "База знаний", icon: "◈" },
  // «Внешние источники» — откуда пополнялся справочник. Отдельно от базы знаний,
  // потому что отвечает на третий вопрос: не «что можно выполнять» и не «чем», а
  // «откуда мы это узнали и что решили по каждой внешней записи».
  { href: "/ingestion", label: "Внешние источники", icon: "◎" },
  { href: "/ai", label: "Искусственный интеллект", icon: "◆" },
  // Состояние компонентов — чтение, поэтому доступно и наблюдателю: удаление
  // записей реестра сервер разрешает только администратору.
