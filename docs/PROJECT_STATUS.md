...
- учётные записи в БД (`admin_users`), пароль как scrypt-хеш из стандартной
  библиотеки (новых зависимостей не добавлено); администратор из переменных
  окружения сохранён как аварийный вход и помечен в интерфейсе;
- роли **admin** и **viewer**; ограничение viewer'а обеспечивает сервер
  (изменяющие endpoint'ы отвечают 403), а не скрытая кнопка;
- CRUD пользователей, смена своего пароля, сброс пароля администратором с
  одноразовым показом временного пароля и обязательной сменой при входе;
- защита от потери доступа: нельзя понизить, отключить или удалить последнего
  активного администратора, нельзя удалить себя;
- задел под вход через Яндекс/VK/MAX: пользователь и способ входа разделены
  (`admin_identities`), `authenticate_external` реализован, привязка аккаунтов
  есть в интерфейсе. **OAuth-флоу не реализованы** и помечены недоступными;
- подробности и осознанные ограничения — `USERS_AND_ACCESS.md`.
...
2. **Нет rate limiting на вход в админку** — перебор пароля ничем не ограничен.
   Учётные записи и роли уже в БД, но защита от brute force остаётся задачей
   Phase 1.3.
...
5. Три пути генерации программы: Telegram идёт через оркестратор с fallback и
   ...
   (Phase 1.2).
6. Часть интеграционных тестов требует наполненного каталога упражнений из
   ...
   делать вручную.
7. `alembic check` сообщает о расхождении ORM-моделей и миграций
