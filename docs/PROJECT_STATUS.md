...
### Gym Knowledge Base & Equipment Intelligence: ГОТОВО

Знание об оборудовании существовало в одном виде: JSON-массив строк в
`exercises.equipment` со значениями источника каталога. Из него нельзя было
ответить ни на один практический вопрос. `machine` не различает жим ногами и
блочную тягу; `other` у 122 упражнений не означает ничего; пустой список у 77
упражнений одинаково читается как «оборудование не нужно» и как «неизвестно».
Добавление тренажёра требовало правки Python-словаря `EQUIPMENT_ALIASES` в
фильтре — словарь оборудования жил в коде, а не в данных.

- **словарь оборудования — данные**: 74 записи, 26 возможностей, 172 синонима
  (миграции `0014`–`0016`). Добавление `Hammer Strength Chest Press` — строка в
  `equipment_items` плюс строки в `equipment_aliases`, а не `if/elif` в
  генераторе;
- **четыре независимых уровня знания**: возможность (что объект умеет) →
  оборудование (что это за объект) → требование упражнения (REQUIRED / OPTIONAL /
  ALTERNATIVE) → профиль (что фактически доступно). Каждый отвечает на свой
  вопрос, и смешивать их значит терять смысл;
- **UNKNOWN ≠ INCOMPATIBLE**. Отсутствие требований у упражнения и отсутствие
  ответа пользователя про тренажёр не являются доказательством несовместимости.
  У наличия оборудования три состояния, у знания — три уровня доверия, у
  результата проверки — три статуса. `assume_unlisted_unavailable` отвечает,
  значит ли отсутствие позиции в профиле «нет» (домашний профиль, где перечислено
  всё) или «неизвестно» (зал, о котором известно только название);
- **специализация оборудования**: `leg_press` → `resistance_machine`. Отношение
  необходимо, потому что источник каталога говорит родовыми словами: без него
  человек с жимом ногами получал бы «не подходит» на упражнение «жим ногами».
  Направлено в одну сторону — частное закрывает родовое, обратное неверно;
- **неявной подмены оборудования нет**. Правило «доступное покрывает возможности
  требуемого» проверялось и отвергнуто: гантели покрывают `free_weight` штанги, и
  жим штанги лёжа оказывался выполнимым с гантелями. Взаимозаменяемость
  выражается в данных явно: требованием возможности либо группой ALTERNATIVE;
- **импорт ничего не теряет молча**: 674 упражнения получили подтверждённые
  требования из значений каталога, 127 — выведенные по названию, 122 значения
  `other` записаны в `unmapped_equipment_values` как незакрытый пробел данных.
  Старое поле `exercises.equipment` не удалено — оно остаётся входом для
  повторного сопоставления;
- **альтернативы различают степень замены**: EXACT (неотличимы по признакам
  каталога), SIMILAR (тот же паттерн, другое оборудование или стабилизация),
  PARTIAL (совпадают мышцы, но не паттерн). 4067 связей для 860 упражнений.
  EXACT требует равенства набора оборудования, а не категории: иначе жим гантелей
  объявлялся бы полной заменой жима штанги;
- **Explorer стал equipment-aware**: фильтры по canonical ID словаря, по
  возможностям, по характеру требования и по состоянию знания. Все они серверные —
  сводятся к набору идентификаторов и попадают в тот же SQL-запрос; серверная
  пагинация сохранена;
- **AI в вычислении совместимости не участвует** и не создаёт идентификаторы:
  все ссылки обязаны существовать в базе;
- **Knowledge Base Health**: полнота 91.75% (801 из 873), orphan-ссылок 0,
  невозможных комбинаций 0, неизвестных возможностей 0. Все числа считаются
  запросами к базе.

Pipeline генерации не менялся: фильтр по-прежнему работает на значениях каталога.
Подключение базы знаний к отбору кандидатов — отдельный шаг, потому что оно
меняет состав пула у всех пользователей и требует собственной приёмки. Отчёт —
`docs/infrastructure/GYM_KNOWLEDGE_BASE_EQUIPMENT_INTELLIGENCE_REPORT.md`.

### External Exercise Knowledge Ingestion: ГОТОВО

Каталог упражнений состоял из одного источника (873 записи). Пополнение означало
ручное добавление, а проверить полноту было нечем — сравнивать не с чем. Механизма
импорта из внешних источников не существовало: `scripts/import_exercises.py`
перезаписывает записи единственного доверенного источника и не содержит ни
сопоставления, ни оценки качества, ни следа происхождения.

Подключены два источника: каталог `hasaneyldrm/exercises-dataset` (1324 записи с
техникой на 10 языках, фотографией и анимацией) и датасет программ Kaggle
`600k-fitness-exercise-and-workout-program` (605 033 строки, 2598 программ).

- **canonical сущность осталась одна**: `exercises`, ключ `(external_id, source)`.
  Второго каталога нет; ни один модуль ingestion не является runtime-зависимостью
  генерации. Знание об источниках живёт в пяти отдельных таблицах (миграция
  `0017`): реестр источников, версии, staging-записи с решением, связи упражнения
  с источниками, происхождение каждого поля, программные наблюдения;
- **датасет Kaggle не является каталогом упражнений.** 605 тысяч строк — это
  вхождения упражнений в программы, а не упражнения: 3211 уникальных названий.
  Трактовка строк как упражнений дала бы 605 тысяч дублей. Техник, описаний и
  медиа в источнике нет — он даёт программный контекст (подходы, повторения,
  интенсивность, цели, уровни) и ничего больше;
- **дедупликация различает три исхода**: то же упражнение, другой вариант того же
  движения, другое упражнение. Тождество решается предикатом, а не порогом: порог
  позволил бы объявить тождеством запись с разошедшимся хватом, набравшую баллы на
  совпадении мышц. `Barbell Bench Press` и `Bench Press - Barbell` — одно
  упражнение; `Barbell Bench Press - Medium Grip` — отдельное. `Dumbbell Biceps
  Curl`, `DB Biceps Curl` и `Dumbbell Curl` — одно;
- **AI в сопоставлении и оценке не участвует.** Результат импорта обязан
  совпадать при повторном запуске на тех же данных; вызов модели этого не
  гарантирует. Веса и пороги — задокументированные константы;
- **отсутствие техники — отказ, а не низкий балл.** Иначе запись с полным набором
  мышц и медиа набрала бы проходной балл, и в программу пользователя попало бы
  упражнение без объяснения, как его выполнять;
- **словарь оборудования пополнен, а не создан заново**: +3 записи (`bosu_ball`,
  `upper_body_ergometer`, `ski_ergometer`), +25 синонимов (9 для новых записей и
  16 для уже существующего оборудования); итого 77 записей и 197 синонимов. Пять значений источника (`weighted`, `assisted`, `rope`, `roller`,
  `hammer`) синонимов не получили: это не названия оборудования, и выбрать
  «наиболее вероятный» снаряд значило бы записать догадку как факт;
- **мышцы приводятся с четырьмя отношениями**, а не «нашлось / не нашлось»:
  тождество, поглощение более общим термином, неоднозначность, отсутствие
  эквивалента. `upper back` (125 записей) не подставляется автоматически: это
  анатомическое утверждение, которого источник не делал;
- **ни одно поле не перезаписывается слепо**: техника берётся только если полнее,
  мышцы дополняются только там, где caталог их не знает, название caталога
  сохраняется, внешнее идёт в синонимы. Каждое изменённое поле получает запись
  происхождения;
- **импорт сходится за один прогон**: 953 создано, 319 обогащено; второй и третий
  прогоны не создают и не меняют ничего. Обеспечено четырьмя механизмами — связь
  источника как факт, неснижаемые роли `origin`/`duplicate_variant`, перепроверка
  плана против фактического состояния, обогащение из дублей источника сразу;
- **результат**: каталог 873 → 1826 упражнений, техника 870 → 1823, русская
  техника 870 → 1823, медиа-ассетов 1746 → 3652 (появились 953 анимации
  выполнения), требования к оборудованию 803 → 1710, альтернатив 4067 → 8257;
- **найден и устранён дефект сортировки**, из-за которого ни одно импортированное
  упражнение не попадало в программу: внешний источник пишет названия строчными, а
  сортировка по названию ставила строчные буквы после всех заглавных. Сортировка
  сделана регистронезависимой в генераторе, фильтре и репозитории; названия
  приведены к регистру caталога при импорте;
- **Admin API и интерфейс — только чтение**: раздел «Внешние источники» с
  источниками, записями и полнотой; в карточке упражнения — раздел «Откуда
  данные». Запуск ingestion остаётся операцией обслуживания, потому что требует
  локальной копии источника, которой у backend нет.

Незакрытым осталось и показано в отчёте: `mechanic` и `difficulty` у 953
импортированных упражнений (источник их не сообщает), 114 упражнений без основных
мышц, 70 записей с родовым оборудованием, 5 записей на проверку человеком. Отчёт —
...
Отдельно от Phase 1.2 остаётся продолжение Gym Knowledge Base: подключение
базы знаний к отбору кандидатов вместо словаря `EQUIPMENT_ALIASES` в коде и
нормализация ответов анкеты в `equipment_profiles`. Оба шага меняют состав пула
упражнений у существующих пользователей, поэтому вынесены в отдельный этап с
собственной приёмкой, а не сделаны вместе с введением модели знаний.

После подключения внешних источников к этому добавились два продолжения,
относящиеся к качеству каталога, а не к его размеру:

- **классификация импортированных упражнений**: `mechanic` (compound/isolation) и
  `difficulty` не заполнены у 953 записей, потому что источник их не сообщает.
  Из-за отсутствия `mechanic` такие упражнения ранжируются ниже canonical
  compound-упражнений, из-за отсутствия `difficulty` доступны любому уровню. Это
  классификация, а не вычисление, и выдумывать её из названия нельзя;
- **закрытие неоднозначных обозначений мышц**: `upper back` (125 записей), `core`
  (94), `hip flexors` (77) указывают на несколько canonical групп. Решение
  анатомическое, и принимать его должен человек; интерфейса для этого пока нет —
  этап дал видимость пробела, а не редактирование staging.
